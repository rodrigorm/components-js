var Tree = Class.create({
  ...
  load: function(element) {
    var container, c, first, last, tree = this, main = {};
    ...
    function visit(element, parent) {
      var con, components = {}, flags = {}, all = [], names = [];
      ...
      for (var name, i = 0; i < all.length; i++) {
        ...
        if (!names._h[name]) { // filter out duplicates
          ...
          if (tree.bindings[name]) {
            con = con || add(new Container(tree, element, names, components, parent));
            components[name] = new tree.bindings[name](con, name);

            if (name == element.id)
              main[name] = components[name];
      ...
      for (var i = 0, node; i < element.childNodes.length; i++) // visit the subtree
        if ((node = element.childNodes[i]).nodeType == 1)
          visit(node, con || parent);

      if (parent && con)  // make parent references
        for (var name in parent.components)
          con.objects[name] || con.set(name, parent.components[name]);     
    ...
    return [container, first, last, main];
  }
});
...
var Container = Class.create({
  ...
  clone: function(deep) {
    return this.tree.load(this.element.cloneNode(deep))[0];
  },
  ...
  build: function(name, data, next) {
    ...
    if (com = this.seek(name, 'next')) {
      while (next = com.next()) com = next;
      return this.insert(com.clone(true), next).update(data || '');
    } else
